 ...
 *         TODO: Description
 */
@Service @SuppressWarnings(value = "deprecation" )
class DefaultTokenDictionaryServiceImpl implements TokenDictionaryService {
	private static final Logger LOG = LoggerFactory.getLogger(DefaultTokenDictionaryServiceImpl.class);
	...
	// @Autowired
	private DefaultTokenDictionaryRepoImpl repo;
	// @Autowired
	private DefaultExternalDataRepoImpl ext;
	 ...
	 */
	@Override
	public synchronized List<TokenDefinitionJpaImpl> get() {
	 ...
	 * 
	 * @see
	 * com.itgfirm.docengine.token.TokenDictionaryService#get(java.lang.Long)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * com.itgfirm.docengine.token.TokenDictionaryService#getDefinitionMap()
	 */
	@Override
	public synchronized Map<String, Map<String, Map<String, Deque<TokenDefinitionJpaImpl>>>> getDefinitionMap() {
	 ...
	 */
	@Override
	public synchronized Map<String, DefaultTokenDataImpl> getTokenDataMap(String projectId) {
		if (Utils.isNotNullOrEmpty(_dictionary)) {
			Map<String, DefaultTokenDataImpl> projectTokens = new HashMap<String, DefaultTokenDataImpl>(get().size());
			...
			while (sqlMap.hasNext()) {
				...
				if (Utils.isNotNullOrEmpty(response)) {
					...
					while (results.hasNext()) {
						...
						Iterator<Entry<String, Map<String, Deque<TokenDefinitionJpaImpl>>>> entities = _definitionMap
								.get(sqlKey).entrySet().iterator();
						while (entities.hasNext()) {
							Iterator<Entry<String, Deque<TokenDefinitionJpaImpl>>> attributes = entities.next()
							...
							while (attributes.hasNext()) {
								...
								while (definitions.hasNext()) {
									...
									if (definition.getAttribute() != null
										...
										DefaultTokenDataImpl data = projectTokens.get(definition.getName());
										...
										if (result.getKey().equals(attributeKey)) {
											data.addValue(new DefaultTokenValueImpl(definition, result.getValue()));
										}
									}
								}
							}
	 ...
	 */
	@Override
	public Map<String, List<DefaultTokenValueImpl>> getTokenValueMap(DefaultProjectImpl project) {
		Map<String, List<DefaultTokenValueImpl>> tokensValuesMap = new HashMap<String, List<DefaultTokenValueImpl>>(
				get().size());
		for (DefaultTokenDataImpl data : getTokenDataMap(project.getProjectNumber()).values()) {
			List<DefaultTokenValueImpl> phaseValues = new ArrayList<DefaultTokenValueImpl>(
					data.getValues(project.getPhase()));
			if (!phaseValues.isEmpty()) {
				tokensValuesMap.put(data.getName(), phaseValues);
			}
		}
		mergeProjectTokenValueMapWithEmptyTokenValueObjects(tokensValuesMap, project.getPhase());
	 ...
	 * 
	 * @see
	 * com.itgfirm.docengine.token.TokenDictionaryService#getDroolsSafeTokens(
	 * com.itgfirm.docengine.pipeline.Project)
	 */
	@Override
	public Map<String, ?> getDroolsSafeTokens(DefaultProjectImpl project) {
		Map<String, List<DefaultTokenValueImpl>> projectTokens = getTokenValueMap(project);
		Map<String, Object> droolsTokens = new HashMap<String, Object>(projectTokens.size());
		for (String key : projectTokens.keySet()) {
			List<DefaultTokenValueImpl> list = projectTokens.get(key);
			Object data = null;
	 ...
	 */
	@Override
	public Map<String, ?> getFreemarkerSafeTokens(DefaultProjectImpl project) {
		Map<String, List<DefaultTokenValueImpl>> valueMap = getTokenValueMap(project);
		if (Utils.isNotNullOrEmpty(project) && Utils.isNotNullOrEmpty(valueMap)) {
			...
			Iterator<Entry<String, List<DefaultTokenValueImpl>>> projectTokens = valueMap.entrySet().iterator();
			while (projectTokens.hasNext()) {
				Entry<String, List<DefaultTokenValueImpl>> entry = projectTokens.next();
				String name = entry.getKey();
				List<DefaultTokenValueImpl> values = entry.getValue();
				DefaultTokenValueImpl value = null;
				String error = null;
				if (values.isEmpty()) {
					error = "[" + DefaultTokenValueImpl.NOT_DEFINED_TXT + " - XXX --- ODD BALL1 EMPTY: " + name + "]";
				} else if (values.size() > 1) {
					error = "[" + DefaultTokenValueImpl.NOT_DEFINED_TXT + " - XXX --- ODD BALL2 MULTIPLES: " + name
							+ "]";
				} else {
					value = values.get(0);
				}
				if (value != null) {
					if (value.getDisplayValue() != null && !value.getDisplayValue().isEmpty()) {
						synchronized (_debugMode) {
							if (_debugMode.get()) {
								freemarkerTokens.put(name, "[" + name + "=" + value.getDisplayValue() + "]");
							} else {
						...
						if (!freemarkerTokens.containsKey(name)) {
							if (error != null) {
								...
								error = "[" + DefaultTokenValueImpl.NOT_DEFINED_TXT
			...
			if (!project.getExhibits().isEmpty()) {
				freemarkerTokens.put("exhibits", Collections.unmodifiableList(project.getExhibits()));
				for (ExhibitImpl exhibit : project.getExhibits()) {
					freemarkerTokens.put("exhibit" + exhibit.getExhibit(), exhibit.getDisplayName());
				}
			}
			String leaseCode = project.getLeaseNumber();
			freemarkerTokens.put("leaseCode", leaseCode);
			freemarkerTokens.put("extendedLeaseCode", String.format("%s %s", project.getDocumentCd(), leaseCode));
	 ...
	 */
	@Override
	public TokenDefinitionJpaImpl merge(TokenDefinitionJpaImpl item) {
		TokenDefinitionJpaImpl result = repo.merge(item);
		refresh();
		return result;
	}
	 ...
	 * 
	 * @see
	 * com.itgfirm.docengine.token.TokenDictionaryService#merge(java.util.List)
	 */
	@Override
	public List<TokenDefinitionJpaImpl> merge(List<TokenDefinitionJpaImpl> tokens) {
		if (Utils.isNotNullOrEmpty(tokens)) {
			LOG.trace("Attempting To Merge List Of Token Definitions: " + tokens.size() + " Items.");
			ListIterator<TokenDefinitionJpaImpl> iter = tokens.listIterator();
			while (iter.hasNext()) {
...
//	@PostConstruct
	...
	private synchronized void refreshTokenDictionary() {
		...
		List<TokenDefinitionJpaImpl> newTokens = repo.get();
		if (Utils.isNotNullOrEmpty(newTokens)) {
			LOG.trace("Found " + newTokens.size() + " Tokens In The Database.");
			_dictionary = Collections.synchronizedList(Collections.unmodifiableList(newTokens));
		} else {
	...
	private synchronized void refreshDefinitionMap() {
		if (Utils.isNotNullOrEmpty(_dictionary)) {
			LOG.trace("Attempting To Refresh The 'Where.Entity' " + "To Entity/Attribute/Definitions Map.");
			Map<String, Map<String, Map<String, Deque<TokenDefinitionJpaImpl>>>> newDefinitionMap = new HashMap<String, Map<String, Map<String, Deque<TokenDefinitionJpaImpl>>>>();
			Iterator<TokenDefinitionJpaImpl> it = get().iterator();
			while (it.hasNext()) {
				TokenDefinitionJpaImpl tokenDefinition = it.next();
				if (tokenDefinition.getEntity() != null && tokenDefinition.getAttribute() != null
						&& tokenDefinition.getWhere() != null) {
					String definitionKey = String.format("%s.%s", tokenDefinition.getWhere(),
							tokenDefinition.getEntity());
					if (newDefinitionMap.get(definitionKey) == null) {
						newDefinitionMap.put(definitionKey, new HashMap<String, Map<String, Deque<TokenDefinitionJpaImpl>>>());
					}
					if (newDefinitionMap.get(definitionKey).get(tokenDefinition.getEntity()) == null) {
						newDefinitionMap.get(definitionKey).put(tokenDefinition.getEntity(),
								new HashMap<String, Deque<TokenDefinitionJpaImpl>>());
					}
					if (newDefinitionMap.get(definitionKey).get(tokenDefinition.getEntity())
						...
						newDefinitionMap.get(definitionKey).get(tokenDefinition.getEntity())
								.put(tokenDefinition.getAttribute(), new ArrayDeque<TokenDefinitionJpaImpl>());
					}
					newDefinitionMap.get(definitionKey).get(tokenDefinition.getEntity())
			...
			if (Utils.isNotNullOrEmpty(newDefinitionMap)) {
				_definitionMap = Collections.synchronizedMap(Collections.unmodifiableMap(newDefinitionMap));
			} else {
				_definitionMap = null;
			}
		} else {
			_definitionMap = null;
			LOG.warn("Cannot Create/Update 'Where.Entity' " + "To Entity/Attribute/Definitions Map! "
	...
	private synchronized void refreshSqlMap() {
		if (Utils.isNotNullOrEmpty(_definitionMap)) {
			...
			Iterator<Entry<String, Map<String, Map<String, Deque<TokenDefinitionJpaImpl>>>>> definitions = _definitionMap
					.entrySet().iterator();
			while (definitions.hasNext()) {
				StringBuilder sb = new StringBuilder();
				Entry<String, Map<String, Map<String, Deque<TokenDefinitionJpaImpl>>>> definition = definitions.next();
				String definitionKey = definition.getKey();
				String entityKey = null;
				String where = null;
				Iterator<Entry<String, Map<String, Deque<TokenDefinitionJpaImpl>>>> entities = definition.getValue().entrySet()
						.iterator();
				while (entities.hasNext()) {
				...
				sb.insert(0, SELECT_) // Insert SELECT in front of the collected
										// attributes
	 ...
	 * @param projectTokens
	 */
	private void mergeProjectTokenDataMapWithEmptyTokenDataObjects(Map<String, DefaultTokenDataImpl> projectTokens) {
		if (Utils.isNotNullOrEmpty(projectTokens)) {
			for (TokenDefinitionJpaImpl definition : get()) {
				if (projectTokens.containsKey(definition.getName())) {
					...
					DefaultTokenDataImpl data = new DefaultTokenDataImpl(definition);
					if (data.getValues().isEmpty()) {
						data.addValue(new DefaultTokenValueImpl(definition, null));
					}
					projectTokens.put(definition.getName(), data);
	 ...
	 * @param phase
	 */
	private void mergeProjectTokenValueMapWithEmptyTokenValueObjects(Map<String, List<DefaultTokenValueImpl>> projectTokens,
			String phase) {
		if (Utils.isNotNullOrEmpty(projectTokens)) {
			for (TokenDefinitionJpaImpl definition : get()) {
				if (phase.equals(definition.getPhase())) {
					List<DefaultTokenValueImpl> tokenValues = projectTokens.get(definition.getName());
					if (tokenValues == null) {
						tokenValues = new ArrayList<DefaultTokenValueImpl>(1);
