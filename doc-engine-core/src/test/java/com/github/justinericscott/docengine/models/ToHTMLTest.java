...
public class ToHTMLTest extends AbstractTest {
	...
	public void a_ParagraphToHTMLTest() {
		...
		String head = HEAD.wrap(title.concat(STYLE.style(TEST_VALUE_CSS_PARAGRAPH, CSS_TYPE_TEXT)));
		...
		head = HEAD.wrap(title.concat(STYLE.style(TEST_VALUE_CSS_PARAGRAPH, CSS_TYPE_TEXT)));
		html = String.format("%s%s", head, body);
		html = doctype().concat(DOCUMENT.wrap(html, null, namespace()));
	...
	private String tidy(final String xhtml, final String path) {
