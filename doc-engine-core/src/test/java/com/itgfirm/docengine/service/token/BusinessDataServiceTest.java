...
import com.itgfirm.docengine.service.token.types.ExternalAttribute;
...
public class BusinessDataServiceTest extends AbstractTest {
	...
	public void xx_MinimalCRUDTest() {
		if (!isSetup) {
			...
			assertEquals(data.concat(data),
					service.queryForMap(sql).entrySet().iterator().next().getValue().toString());
	...
	private void setup() {
		if (isNotNullOrEmpty(service)) {
			final ExternalSchema schema = service.getExternalSchema();
