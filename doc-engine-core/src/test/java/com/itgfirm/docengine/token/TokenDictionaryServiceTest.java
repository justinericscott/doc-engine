...
public class TokenDictionaryServiceTest extends AbstractTest {
	private static final Logger LOG = LogManager.getLogger(TokenDictionaryServiceTest.class);	
	...
	public void aa_MergeTest() {
		// Merge 1
		TokenDefinition token = createFakeTokenDefinition();
		tokenDictionaryService.delete(token);
		
		List<TokenDefinition> tokens = new ArrayList<TokenDefinition>();
	...
	public void ab_MergeInvalidParamTest() {
		// Null Object
		assertNull(tokenDictionaryService.merge((TokenDefinition) null));
		// Empty Code
		...
		// Empty Name
		assertNull(tokenDictionaryService.merge(new TokenDefinitionJpaImpl(TestUtils.getRandomTestString(1), "")).getId());
		// Duplicate Codes
		TokenDefinition token = createFakeTokenDefinition();
	...
	public void ba_GetTest() {
		...
		List<TokenDefinition> allDefs = tokenDictionaryService.get();
		...
		TokenDefinition def = createFakeTokenDefinition(6);
	...
	public void ca_GetWithCodeLikeTest() {
		TokenDefinition def = createFakeTokenDefinition(7);
	...
	public void da_GetProjectTokenDataMap() {
		...
		setupExternalSchema();
		List<TokenDefinition> tokens = createRealTokenDefinitions();
		assertEquals(tokens.size(), tokenDictionaryService.get().size());
		LOG.debug("Generating Project TokenData Map For: " + projectId);
		
		Iterator<Entry<String, TokenData>> tokendataMap = tokenDictionaryService.getTokenDataMap(projectId).entrySet().iterator();
		while (tokendataMap.hasNext()) {
			Entry<String, TokenData> t = tokendataMap.next();
			String key = t.getKey();
			TokenData data = t.getValue();
			LOG.debug("Checking TokenData: " + data.getName());
			for (TokenValue v : data.getValues()) {
				LOG.debug("Project TokenData Map Result Key: " + key 
						+ " | Value: " + v.getDefinition().getTokenCd() + ": " + v.getDisplayValue());	
			}			
		}
		for (TokenDefinition d : tokens) {
	...
	public void eb_RefreshWhereEntityMapTest() {
		...
		Iterator<Entry<String, Map<String, Map<String, Deque<TokenDefinition>>>>> whereMap = tokenDictionaryService.getDefinitionMap().entrySet().iterator();
		while (whereMap.hasNext()) {
			...
			Iterator<Entry<String, Map<String, Deque<TokenDefinition>>>> entities = whereMapEntry.getValue().entrySet().iterator();
			while (entities.hasNext()) {
				...
				Iterator<Entry<String, Deque<TokenDefinition>>> attributes = entity.getValue().entrySet().iterator();
	...
	private TokenDefinition createFakeTokenDefinition(int seed) {
		TokenDefinition token = tokenDictionaryService.merge(makeTestTokenDefinition(seed));
		validate(token);
		return token;
	}
	
	private List<TokenDefinition> createRealTokenDefinitions() {
		// Project Number
		TokenDefinition projectNumberRlp = new TokenDefinitionJpaImpl(TestConstants.TEST_CODE_PROJECT_NBR_R101, TestConstants.TEST_NAME_PROJECT_NUMBER);
		...
		TokenDefinition projectNumberPropLease = new TokenDefinitionJpaImpl(projectNumberRlp, TestConstants.TEST_CODE_PROJECT_NBR_L201_PROPOSED);
		...
		TokenDefinition projectNumberAwardLease = new TokenDefinitionJpaImpl(projectNumberPropLease, TestConstants.TEST_CODE_PROJECT_NBR_L201_AWARDED);
		projectNumberAwardLease.setPhase(TestConstants.TEST_PHASE_AWARDED);		
		
		// Lease Number
		TokenDefinition leaseNumberRlp = new TokenDefinitionJpaImpl(TestConstants.TEST_CODE_LEASE_NBR_R101, TestConstants.TEST_NAME_LEASE_NUMBER);		
		...
		TokenDefinition leaseNumberPropLease = new TokenDefinitionJpaImpl(leaseNumberRlp, TestConstants.TEST_CODE_LEASE_NBR_L201_PROPOSED);
		...
		TokenDefinition leaseNumberAwardLease = new TokenDefinitionJpaImpl(leaseNumberPropLease, TestConstants.TEST_CODE_LEASE_NBR_L201_AWARDED);
